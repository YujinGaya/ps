...
///
/// # 좋은 점: 자연스러운 문자열 비교
/// C/C++에서 함수를 통해 부자연스럽게 문자열을 비교해야하는 것과 다르게, Rust에서는 &str 이나 &[char]는 == 연산자로 비교할
/// 수 있다.
///
/// # 한계: 스타일
/// Rust는 usize 타입끼리의 연산에서 오버플로우가 일어나면 패닉을 하고 죽기 때문에 (2)처럼 커서를 부자연스럽게 조절해야하는 문제가
/// 있었다. end가 "현재까지 확정된 문자열의 끝 인덱스"로 생각하게 되면 찾으려고 하는 문자열의 prefix가 explosions인 경우에
/// 확정된 문자열이 없는 경우, 즉 끝 인덱스도 정의할 수 없는 상태가 된다. C였다면 잠시 오버플로우가 일어났다가 loop의 끝에
/// index를 1씩 더할 때 다시 0이 될 문제인데, Rust에서는 패닉이 일어나 아래처럼 구현했다.
///
/// # 한계: 소유권 우회
